 ...
 * @type {{isAuthenticate: (function(): boolean), getCookie: (function(*=): *)}}
 */

window.ssoMeConnect = {
  isAuthenticate: function() {
    var sso_session_id = Cookies.get("sso_session_id");
    if (typeof sso_session_id == 'undefined') {
      localStorage.removeItem("sso_token");
      return false;
    }
    var sso_token = drupalSettings.sso_token;
    var local_sso_token = localStorage.getItem("sso_token");
    // Update localstorage if local value is null and sso_token from drupalSettings has value
    if ( local_sso_token == null && typeof sso_token !== 'undefined') {
      localStorage.setItem("sso_token", sso_token);
      drupalSettings.sso_token = null;
    }
    if (local_sso_token == null) {
      return false;
    }
    return true;
  },
  ...
  getDrupalCSRFToken: async function() {
    var url = window.location.protocol + "//" +window.location.hostname + "/session/token"
  ...
  userInfoUpdateApi: async function(action, url, data) {
    let updateStatus = {};
    const result = await ssoMeConnect.userInfoPostApiDrupal('POST', url, data);
  ...
  userInfoPostApi: async function(method, url, data = []) {
    var session_token = localStorage.getItem('sso_token');
    if (method === 'POST') {
    ...
    const res = await fetch(url, {
      ...
      headers: {
        ...
        'Access-Control-Request-Headers': 'Authorization, Content-Type, X-PINGOTHER'
      },
      body: bodyContent,
    });
  ...
  userInfoCheckFollow: async function(uuid, url) {
    var isfollowing = false;
    const data = await ssoMeConnect.userInfoPostApi('GET', url);
    if (!Array.isArray(data.subscriptions.topics)) {
      return isfollowing;
    }
    else {
      var topics = data.subscriptions.topics;
      topics.forEach( function(item, index) {
        if (item.uuid === uuid && item.isDeleted === false) {
          isfollowing = true;
          return isfollowing;
        }
      });
  ...
  Request: async function(method, url, uuid = null, data = []) {
    if (method === 'POST') {
      bodyContent = JSON.stringify(data);
    }
    else {
      bodyContent = null;
      url = url + ((uuid != null) ? "?uuid=" + uuid : "");
    }
    const res = await fetch(url, {
      method: method,
      body: bodyContent,
  ...
  userInfoPostApiDrupal: async function(method, url, data=[]) {
    var X_CSRF_Token = await ssoMeConnect.getDrupalCSRFToken();

    if (method === 'POST') {
      var dataWithtoken = {};
      dataWithtoken.data = data;
      var bodyContent = JSON.stringify(dataWithtoken);
    }
    else { bodyContent = null;}
  ...
  updatelocalStorge: function(key, action, uuid, bundle = null){
    let currentItems =  ssoMeConnect.getlocalStorage(key);
    if (bundle && (key === 'meconnectSubscriptions')) {
      currentItems = JSON.parse(currentItems);
      let items = (currentItems[bundle]) ? (currentItems[bundle]) : [];

      if(action === 'addItem' && !items.includes(uuid)) {
        items.push(uuid);
      }
      else{
        let indexOfUuid = items.indexOf(uuid);
        items.splice(indexOfUuid, 1);
      }
      ...
      if(action === 'addItem' && !currentItems.includes(uuid)) {
        currentItems.push(uuid);
      }
      else{
        let indexOfUuid = currentItems.indexOf(uuid);
        currentItems.splice(indexOfUuid, 1);
      }
    }
    ssoMeConnect.deletelocalStorage(key);
    ssoMeConnect.setlocalStorage(key, currentItems);
  },

  getSubscriptionUuidMapping(SubscriptionList) {
    let currentSubscriptions = {};
    if (SubscriptionList) {
      for (let [key, value] of Object.entries(SubscriptionList)) {
        if (value) {
          currentSubscriptions[key] = value.map(elem => elem.uuid);
        }
      }
    }
  ...
  getsubscriptionKeyFromBundle(bundle) {
    let keyBundleMapping = {
      'topic': 'topics',
      'author':  'authors',
      'category': 'categories',
      'watch_program': 'video_programs',
      'omnystudio_programs': 'audio_programs'
    };
    return keyBundleMapping[bundle];
  }
};

  // Fix for meconnect session.

  document.onreadystatechange = function () {

    // Set localstorage just after login.
    if (ssoMeConnect.getCookie('sso_login') === "1") {
      ssoMeConnect.setlocalStorage('meconnectSessionId', ssoMeConnect.getCookie('sso_session_id'));
      ssoMeConnect.deleteCookie('sso_login');
      ssoMeConnect.setlocalStorage("user_info_username", ssoMeConnect.getCookie('meConnectUser'));
      ssoMeConnect.deleteCookie('meConnectUser');
    }

    if (ssoMeConnect.getlocalStorage('meconnectSessionId') !== ssoMeConnect.getCookie('sso_session_id')) {
      ssoMeConnect.deletelocalStorage('meconnectSessionId');
      ssoMeConnect.deleteCookie('sso_token');
      ssoMeConnect.deleteCookie('sso_id');
      ssoMeConnect.deleteCookie('sso_session_id');
      ssoMeConnect.deleteCookie('sso_avatar');
      ssoMeConnect.deleteCookie('sso_source');
      ssoMeConnect.deleteCookie('created_date');
      ssoMeConnect.deleteCookie('issued_date');
    }

    $(document).on('click', 'a.logout', function(evt) {
      evt.preventDefault();
    ...
    /**
     * Function to create local storage for userinfo.
     */
    $.fn.user_info_localStorage = function (username, sso_token) {
      ssoMeConnect.setlocalStorage("user_info_username", username);
      ssoMeConnect.setlocalStorage("sso_token", sso_token);
    };

    /**
     * Function to clear local storage for userinfo.
     */
    $.fn.user_clean_localStorage = function () {
  ...
  var login_param = getSearchParams("login");
  if (login_param != '' && login_param == 'acc-deleted') {
    setTimeout(function() {
      var get_cookie = Cookies.get('sso_account_deleted_popup');
      if (typeof get_cookie !== 'undefined') {
        if (!$('#acc-deleted').hasClass('acc-delete-popup')) {
          var $previewDialog = $(drupalSettings.sso_account_deleted_msg).appendTo('body');
          Drupal.dialog($previewDialog, {
            close: function (event) {
              Cookies.remove("sso_account_deleted_popup");
            }
          }).showModal();
        }
      }
  ...
  // Reload the captcha after validation message poped up in meconnect login page.
  $.fn.grecaptcha_reload = function grecaptcha_reload() {
    grecaptcha.reset();
  }
...
;
(function ($, Drupal) {
  Drupal.behaviors.meConnectRefreshToken = {
    attach: function (context) {
      $('main', context).once('meConnectRefreshToken').each(function () {
        var issuedDataCookie = ssoMeConnect.getCookie('issued_date');
        if (typeof issuedDataCookie !== "undefined") {
          var issuedDataCookieInt = parseInt(issuedDataCookie);
          var issuedDate = new Date(issuedDataCookieInt*1000).toISOString().substr(0,10);
          var currentDate = new Date().toISOString().substr(0,10);
          var diff =  Math.floor(( Date.parse(currentDate) - Date.parse(issuedDate) ) / 86400000);
          if (diff > 5) {
            $.ajax({
              async:false,
              url: window.location.origin + '/profile/refresh-token',
              type: 'GET',
              dataType: 'json',
              success: function() {
                localStorage.setItem("sso_token", data);
                console.log("SSO Token now Updated.");
              },
              error: function (xhr, status, error) {
                console.log(error);
                console.log("SSO Token NOT Updated.");
              }
            });
          }
        }
